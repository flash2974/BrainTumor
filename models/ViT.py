from keras.layers import Conv2D, Dense, Dropout, LayerNormalization, MultiHeadAttention, RandomFlip, RandomRotation, RandomZoom, Reshape, Layer, Add
...
class ViT(Model):
    def __init__(self, 
        ...
        self.blocks = Sequential([
            TransformerBlock(nb_heads, embedding_dimension, dropout_rate) for _ in range(nb_blocs)
        ], name="transformer_encoder")
        
        self.norm = LayerNormalization(epsilon=1e-6)
        self.class_head = Dense(nb_classes, activation="softmax", name="classification_head")
